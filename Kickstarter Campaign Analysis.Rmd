...
author: "Norris Jaber, Rajeev Agrawal, Sha Lu, Yuxi Duan"
date: "12/2/2021"
output: 
  pdf_document:
...
for (i in 1:20) {
  DC.knn <- knn(kick.normal_train, kick.normal_test, kick.normal.labels_train, k=i)
  error.rate <- 1-(sum(DC.knn == kick.normal.labels_test) / length(kick.normal.labels_test))
...
#prediction
kick_pred3 <- predict(kick_tree, tibble(kick_test))
```

rf prediction
```{r}
rf_fit_test <- fit(rf_spec, status ~ ., data = kick_test)

rf_fit_test %>%
  augment(new_data = kick_test) %>%
  accuracy(status, .pred_class)

kick_predrf <- predict(rf_fit, tibble(kick_test))
```
Rajeev
```{r}
...
```{r}
par(pty="s") 
ROCrf <- roc(kick_test$status ~ kick_predrf, plot=TRUE, print.auc=TRUE, 
               col="green", lwd =2, print.auc.y=0.6, print.auc.x=0.3, legacy.axes=TRUE,
               main="ROC Curves for prediction models")
```
